...
func GetRecordById(recordId string) (bool, int, []responsemode.RecordResult) {
	...
	err := datasource.Engine.SQL(fmt.Sprintf(`select r.record_img, r.emoj_id, r.record_desc, date_format(r.create_time, '%%Y年%%m月%%d日 %%H:%%i') as str_create_date, a.arena_name, a.arena_location from record r left join  arena a on r.arena_id = a.arena_id and a.enable = 1 where r.enable = 1 and  r.record_id = '%v' `, recordId)).Find(&datas)
	if err != nil {
...
func GetRecordByMonth(strDate string) (bool, int, []responsemode.RecordResult) {
	datas := make([]responsemode.RecordResult, 0)
	err := datasource.Engine.SQL(fmt.Sprintf(`select r.record_id, r.emoj_id, r.record_img, r.record_desc, date_format(r.create_time, '%%Y-%%m-%%d %%H:%%i:%%s') as str_create_date, a.arena_name, a.arena_location from record r  left join  arena a on r.arena_id = a.arena_id and a.enable = 1 where r.enable = 1 and  DATE_FORMAT(r.create_time,'%%Y-%%m') = '%v' `, strDate)).Find(&datas)
	if err != nil {
...
func GetRecordByYear(strDate string) (bool, int, []responsemode.RecordResult) {
	datas := make([]responsemode.RecordResult, 0)
	err := datasource.Engine.SQL(fmt.Sprintf(` select r.record_id, r.emoj_id, r.arena_id from  record r where r.enable = 1 and  DATE_FORMAT(r.create_time,'%%Y') = '%v' ORDER BY r.create_time`, strDate)).Find(&datas)
	if err != nil {
